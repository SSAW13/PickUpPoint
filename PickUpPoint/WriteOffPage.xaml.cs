...
public partial class WriteOffPage : ContentPage
{
    ...
    private async void SearchOrderClicked(object sender, EventArgs e)
    {
        ...
        {
            string query = "select * from ordersOnWarehouseForWriteOff where id = " + OrderIdEntry.Text + ";";
    ...
    private async void WriteOffOrder(object sender, EventArgs e)
    {
        string query = "select WriteOffOrder(" + dataTable.Rows[0][0] + ", " + dataTable.Rows[0][1] + ")";
        DataTable dataTable1 = _databaseConnection.ExecuteQuery(query);
        await DisplayAlert("Успешно", "Заказ списан", "ОК");
        Order.IsVisible = false;
    }
